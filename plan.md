...
| #25 | fix/routing-gh-pages-404-html              | 3.5         | `postbuild` copies `dist/index.html` to `dist/404.html` for GitHub Pages deep links                                                                                            |
| #28 | hotfix/homepage-redesign-and-reader-polish | — (hotfix)  | Editorial featured hero, category chips, feed-only home; navbar category row owned by Navbar; cream page background; related stories on reader                                 |
| #29 | fix/post-card-cta-entities-excerpt         | HF3         | Single `Post` card CTA; `stripHtmlTags` decodes HTML entities; excerpt paragraph omitted when empty (no hardcoded fallback)                                                    |
| #31 | fix/related-posts-app-shell                | —           | Related stories ordered by newest date; `#app` uses brand-white like `body`; plan documents A4/A5 overlap with merged homepage/reader work                                     |
