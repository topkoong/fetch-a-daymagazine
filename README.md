...
| Inline / JSDoc                            | Heavier on **data and behavior** than on every presentational component: WordPress APIs (`src/apis/`), magazine + category fallbacks, related-post selection, SEO JSON-LD (`src/utils/structured-data.ts`, `src/types/seo.types.ts`), and a few shared hooks/constants. **Not** every `.tsx` file has a block comment—prefer clear names and small modules over noise. |
